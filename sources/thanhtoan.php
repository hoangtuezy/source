...
if(!empty($_POST)){
	...
	if(!empty($_POST['phivanchuyen'])){
		...
		if (get_magic_quotes_gpc()==false) {
			...
			$hoten = magic_quote($sender_hoten);

			$dienthoai = magic_quote($sender_dienthoai);

			$diachi = magic_quote($sender_diachi);

			$email = magic_quote($sender_email);

			$noidung = magic_quote($sender_noidung);
		...
		if ($coloi==FALSE) {
			...
			$sql = "INSERT INTO  table_order (madonhang,hoten,dienthoai,diachi,email,tonggia,noidung,ngaytao,trangthai,id_thanhpho,id_quanhuyen,phivanchuyen,pt_thanhtoan,pt_nhanhang,receiver,sender,payment,send_time)

			VALUES ('$mahoadon','$hoten','$dienthoai','$diachi','$email','$tonggia','$noidung','$ngaydangky','1','$sender_id_thanhpho','$sender_id_quanhuyen','$phivanchuyen','$pt_thanhtoan','$pt_nhanhang','$data_receiver_json','$data_sender_json','$payment','$send_time')";
			$d->reset();
			$d->query($sql);
