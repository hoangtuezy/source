...
function _lang($key){
	global $_lang;
	return empty($_lang[$key])?$key:$_lang[$key];
}
function pagination_ajax($query, $per_page = 10, $page = 1, $url = '?')
{
    ...
    $pagination = "";
    if ($lastpage > 1) {
        $pagination .= "<ul class='pagination' data-target='$url'>";
        
        if ($page > 1) {
            $pagination .= "<li class='page-item'><a class='page-link' data-page='{$prev}'>{$prevlabel}</a></li>";
        }
        if ($lastpage < 7 + ($adjacents * 2)) {
            ...
            if ($page < 1 + ($adjacents * 2)) {
                ...
                $pagination .= "<li class='page-item'><a class='page-link' data-page='{$lpm1}'>{$lpm1}</a></li>";
                ...
                $pagination .= "<li class='page-item'><a class='page-link' data-page='2'>2</a></li>";
                $pagination .= "<li class='dot'>...</li>";
                ...
                $pagination .= "<li class='page-item'><a class='page-link' data-page='page=1'>1</a></li>";
                $pagination .= "<li class='page-item'><a class='page-link' data-page='page=2'>2</a></li>";
                $pagination .= "<li class='dot'>..</li>";
        ...
        if ($page < $counter - 1) {
            // $pagination .= "<li class='page-item'><a class='page-link' data-page='{$next}'>{$nextlabel}</a></li>";
            // $pagination .= "<li class='page-item'><a class='page-link' data-page='$lastpage'>{$lastlabel}</a></li>";
        }
        $pagination .= "</ul>";
    }
    return $pagination;
}
function price($giaban,$sdt='',$ccy=' đ'){
    if($giaban==0 || $giaban==''){
...
function layout_mxh($w,$h,$type='mxh_top',$msg=''){
	$_mxh = result_array("select photo,ten,url from #_lkweb where hienthi=1 and type='$type' order by id desc");
	$html  = '<div class="'.$type.' d-flex flex-wrap">';
	if(!empty($msg)){
		$html .= '<div class="label">'.$msg.'</div>';
	}
...
function layout_fanpage($fanpage,$w=500,$h=245){
	$html = '<div class="fb-page" data-href="'.$fanpage.'" data-width="'.$w.'"
  data-height="'.$h.'" data-tabs="timeline" data-small-header="false" data-adapt-container-width="true" data-hide-cover="false" data-show-facepile="true"><blockquote cite="'.$fanpage.'" class="fb-xfbml-parse-ignore"><a href="'.$fanpage.'">'.$fanpage.'</a></blockquote></div>';
  return $html;
}

function layout_bando($bando){
  $html  = '<div class="bando">';
  $html .= $bando;
  $html .= '</div>';	
  return $html;
}
...
function hethang($productid)
{
	global $d;
	$d->reset();
	$sql = "select id from #_thuoctinh where type='mausac' and id_list='".$productid."'";
	$d->query($sql);
	$mausac = $d->result_array();
	foreach ($mausac as $val) {
		$d->reset();
		$sql = "select soluongnhap from #_thuoctinh where type='size' and id_list='".$val['id']."'";
		$d->query($sql);
		$size = $d->result_array();
		foreach ($size as $v) {
			$ketqua+=$v['soluongnhap'];
		}
	}
		return $ketqua;
}
...
function get_tong_tien($id=0){
		...
		if($id>0){
			$d->reset();
			$sql="select gia,soluong,phiship from #_order_detail where id_order='".$id."'";
			$d->query($sql);
			...
			$d->reset();
			$sql="select phivanchuyen from #_order where id='".$id."'";
			$d->query($sql);
			...
			if($tongtien<$row_setting['freeship']){
				$tongtien=$tongtien+$row_setting['phiship'];
			}
			
			$d->reset();
			$sql="select phantram,giatu,giaden from #_gia where hienthi=1";
			$d->query($sql);
			$phantram=$d->result_array();
			foreach($phantram as $val) {
				if(($tongtien>=$val['giatu']) && ($tongtien<=$val['giaden'])){
					$tongtien=$tongtien-(($val['phantram']*$tongtien)/100);
				}
			}
			// return $tongtien+$phivanchuyen['phivanchuyen'];
			return $tongtien;
		}else return 0;
	}
function get_tong_tien2($id=0){
		global $d;
		if($id>0){
			$d->reset();
			$sql="select gia,soluong,phiship from #_order_detail where id_order='".$id."' and id_thanhvien='".$_SESSION['login']['id']."'";
			$d->query($sql);
			$result=$d->result_array();
			$d->reset();
			$sql="select phitinh from #_order where id='".$id."'";
			$d->query($sql);
			$phitinh=$d->fetch_array();
			$tongtien=0;
			for($i=0,$count=count($result);$i<$count;$i++) {
			
					$tongtien+=	($result[$i]['gia']*$result[$i]['soluong'])+$result[$i]['phiship'];
			
			}
			return $tongtien+$phitinh['phitinh'];
		}else return 0;
	}
function get_giamgia($id=0){
	global $d;
	if($id>0){
		$d->reset();
		$sql="select gia,soluong,phiship,id_order from #_order_detail where id='".$id."'";
		$d->query($sql);
		$result_tongtien=$d->result_array();
		$tongtien=0;
		for($i=0,$count=count($result_tongtien);$i<$count;$i++) {
		
				$tongtien+=	($result_tongtien[$i]['gia']*$result_tongtien[$i]['soluong']);
		...
		foreach($phantram as $val) {
			if(($tongtien>=$val['giatu']) && ($tongtien<=$val['giaden'])){
				$giamgia=round(($val['phantram']*$tongtien)/100);
...
function get_total_ship($id=0){
	global $d;
	if($id>0){
		$d->reset();
		$sql="select phiship from #_order_detail where id_order='".$id."'";
		$d->query($sql);
		$result=$d->result_array();
		$tongtien=0;
		for($i=0,$count=count($result);$i<$count;$i++) {
		
				$tongtien+=	$result[$i]['phiship'];
		
		}
		return $tongtien;
	}else return 0;
}
function get_total_ship2($id=0){
	global $d;
	if($id>0){
		$d->reset();
		$sql="select phiship from #_order_detail where id_order='".$id."' and id_thanhvien='".$_SESSION['login']['id']."'";
		$d->query($sql);
		$result=$d->result_array();
		$d->reset();
		$sql="select phitinh from #_order where id='".$id."'";
...
return $result["dangxem"]; // array('dangxem'=>'', 'daxem'=>'')
}
	...
	function pagination($query, $per_page = 10, $page = 1, $url = '?')
{
   ...
   $prevlabel  = "&lsaquo;Trang Trước";
    ...
    $pagination = "";
    if ($lastpage > 1) {
        $pagination .= "<ul class='pagination justify-content-center m-y-5'>";
        $pagination .= "  <li class='page-item disabled'><span class='page-link'>{$page} of {$lastpage}</span></li>";
        if ($page > 1) {
...
function _link_short_name($media){
	global $config_short_link,$link_short_format;
	if($config_short_link){
		return sprintf($link_short_format,$media); 
	}else{
		die("Short link not enable.");
	}
}
function _link_media($com,$media){
	global $config_short_link,$link_media_format;
	if($config_short_link){
		return _link_short_name($media);
	}
    return sprintf($link_media_format,$com,$media);
}
function _link_com($com){
	global $config_short_link,$link_com_format;
    return sprintf($link_com_format,$com);
}
function _link_list($com,$list_name){
	global $config_short_link,$link_list_format;
	if($config_short_link){
		return _link_short_name($list_name);
	}
    return sprintf($link_list_format,$com,$list_name);
}
function _link_cat($com,$list_name,$cat_name){
	global $config_short_link,$link_cat_format;
	if($config_short_link){
		return _link_short_name($cat_name);
	}
    return sprintf($link_cat_format,$com,$list_name,$cat_name);
}
function _link_item($com,$list_name,$cat_name,$item_name){
	global $config_short_link,$link_item_format;
	if($config_short_link){
		return _link_short_name($item_name);
	}
    return sprintf($link_item_format,$com,$list_name,$cat_name,$item_name);
}
function get_tinhthanh_name($pid){
		global $d, $row,$lang;
		$lang = $_SESSION['lang'];
		$sql = "select * from #_city_list where id='".$pid."'";
		$d->query($sql);
		$row = $d->fetch_array();
		return $row['ten_'.$lang];
	}
function get_quanhuyen_name($pid){
		global $d, $row,$lang;
		$lang = $_SESSION['lang'];
		$sql = "select * from #_city_cat where id='".$pid."'";
		$d->query($sql);
		$row = $d->fetch_array();
		return $row['ten_'.$lang];
	}
