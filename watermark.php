...
define('_watermark','upload/watermark.png');
/**
...
if($_GET['w'] < 400){
    if(! defined('WATERMARK_OVERLAY_IMAGE') ) define ('WATERMARK_OVERLAY_IMAGE', _watermark ); // the watermark image
} else {
    if(! defined('WATERMARK_OVERLAY_IMAGE') ) define ('WATERMARK_OVERLAY_IMAGE', _watermark ); // the watermark image
}
...
if(! defined('FILE_CACHE_DIRECTORY') )      define ('FILE_CACHE_DIRECTORY', './water@#cache');               // Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
...
class timthumb {
    ...
    protected function processImageAndWriteToCache($localImage){
        ...
        if ($watermark == 1) {
            $logododungdau=_watermark;
            $overlay_gd_image = imagecreatefrompng($logododungdau);
