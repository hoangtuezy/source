...
function get_mans(){
	...
	if($_REQUEST['noibat3']!='')
	{
		$id_up = $_REQUEST['noibat3'];
		$sql_sp = "SELECT id,noibat3 FROM table_product where id='".$id_up."' ";
		$d->query($sql_sp);
		$cats= $d->result_array();
		$time=time();
		$hienthi=$cats[0]['noibat3'];
		if($hienthi==0)
		{
			$sqlUPDATE_ORDER = "UPDATE table_product SET noibat3 =1 WHERE  id = ".$id_up."";
			$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
		}
		else
		{
			$sqlUPDATE_ORDER = "UPDATE table_product SET noibat3 =0  WHERE  id = ".$id_up."";
			$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
		}	
	}
	#----------------------------------------------------------------------------------------
...
function save_man(){
	...
	$file_qc_name=images_name($_FILES['quangcao']['name']);
	if(empty($_POST)) transfer("Không nhận được dữ liệu", $_SESSION['links_re']);
	$id = isset($_POST['id']) ? themdau($_POST['id']) : "";
	// $a = magic_quote(json_encode($_POST['variables'],JSON_UNESCAPED_UNICODE));
	// dump(json_decode(str_replace('\\', '', $a)),1);
	if($id){
		...
		if($photo_qc = upload_image("quangcao", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_product,$file_qc_name)){
			$data['file'] = $photo_qc;
			//dongdauanh($photo,_upload_product);	
			$d->setTable('product');
			$d->setWhere('id', $id);
		...
		if(!empty($_POST['mausac'])){
			$data['color'] = implode(',',$_POST['mausac']);
		}
		if(!empty($_POST['price_bycolor'])){
			foreach($_POST['price_bycolor'] as $stt => $val){
				$data['price_bycolor'][$_POST['mausac'][$stt]] = str_replace(',','',$val);
			}

		}

		$data['price_bycolor'] = json_encode($data['price_bycolor']);
		...
		$data['variables'] = json_encode($_POST['variables'],JSON_UNESCAPED_UNICODE);
		...
		$data['stock'] = isset($_POST['stock']) ? 1 : 0;
		$data['ngaysua'] = time();
		$d->setTable('product');
		$d->setWhere('id', $id);
		if($d->update($data)){
