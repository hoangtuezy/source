...
function save_item(){
	...
	if($id){ // cap nhat
		...
		if($file_d = upload_image("file_download", _img_type, _upload_download,$file_download)){
			$data['file'] = $file_d;	
			$d->setTable('download');
			$d->setWhere('id', $id);
			$d->select();
		...
		if($file_d = upload_image("file_download", 'rar|zip|xls|xlsx|pdf|doc|docx|ppt|pptx', _upload_download,$file_download)){
			$data['file'] = $file_d;
		}
		$data['ten'] = $_POST['ten'];		
		$data['url'] = $_POST['url'];
