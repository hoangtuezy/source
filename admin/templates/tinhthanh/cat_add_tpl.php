...
<form name="supplier" id="validate" class="form" action="index.php?com=tinhthanh&act=save_cat<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
	<div class="widget">
        ...
        <div class="formRow">
			...
			<div class="formRight">
                <input type="text" name="ten" title="Nhập tên tỉnh thành" id="ten" class="tipS validate[required]" value="<?=@$item['ten_vi']?>" />
			</div>
			<div class="clear"></div>
		</div>
    <div class="formRow">
  ...
  <div class="formRight">
            <input type="text" name="phivanchuyen" title="Nhập phí vận chuyển" id="phivanchuyen" class="tipS conso" value="<?=@$item['phivanchuyen']?>" />
  </div>
  <div class="clear"></div>
</div>
        <div class="formRow">
