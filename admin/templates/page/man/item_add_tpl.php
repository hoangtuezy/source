...
function get_main_list()
{
 ...
 $str='
 <select id="id_list" name="id_list" data-level="0" data-type="'.$_GET['type'].'" data-table="table_page_cat" data-child="id_cat" class="main_select select_danhmuc form-control">
...
{
 global $item;
 $sql="select * from table_page_cat where id_list='".$item['id_list']."' and type='".$_GET['type']."' order by stt asc";
 $stmt=mysql_query($sql);
 $str='
 <select id="id_cat" name="id_cat" data-level="1" data-type="'.$_GET['type'].'" data-table="table_page_item" data-child="id_item" class="main_select select_danhmuc form-control">
...
{
 global $item;
 $sql="select * from table_page_item where id_cat='".$item['id_cat']."' and type='".$_GET['type']."' order by stt asc";
 $stmt=mysql_query($sql);
 $str='
 <select id="id_item" name="id_item" data-level="2" data-type="'.$_GET['type'].'" data-table="table_page_sub" data-child="id_sub" class="main_select select_danhmuc form-control">
...
{
 global $item;
 $sql="select * from table_page_sub where id_item='".$item['id_item']."' and type='".$_GET['type']."' order by stt asc";
 $stmt=mysql_query($sql);
 $str='
...
<div class="wrapper">
  ...
  <div class="control_frm breadcrumb" style="margin-top:25px;">
    <div class="bc">
      <ul id="breadcrumbs" class="breadcrumbs">
       <li><a <?php if($_GET['type']!="phuongcham") { ?>href="index.php?com=page&act=add<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>"<?php } ?>><span>Thêm <?=$title_main?></span></a></li>
 ...
 <form name="supplier" id="validate" class="form" action="index.php?com=page&act=save<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
   <div class="widget">
...
foreach($config_lang as $key => $value) { ?>
  <div class="formRow lang_hidden lang_<?=$key?> <?=$key=='vi'?'active':''?>">
    ...
    <div class="formRight">
      <input type="text" name="<?='ten_'.$key?>" title="Nhập <?=strtolower($txt_title)?>" id="<?='ten_'.$key?>" class="tipS form-control  <?=$key=='vi'?'validate[required]':''?>" value="<?=@$item['ten_'.$key]?>" />
    </div>
...
<?php if($config_link=='true'){ ?>
  <div class="formRow">
   <label>Link</label>
   <div class="formRight">
    <input type="text" name="link" title="Nhập link" id="link" class="form-control tipS" value="<?=@$item['link']?>" />
  </div>
  <div class="clear"></div>
</div>
<?php } ?>
...
<div class="formRow">
  ...
  <div class="formRight">
    ...
    <input type="text" class="form-control tipS" value="<?=isset($item['stt'])?$item['stt']:1?>" name="stt" style="width:40px; text-align:center;" onkeypress="return OnlyNumber(event)" original-title="Số thứ tự của danh mục, chỉ nhập số">
  </div>
  <div class="clear"></div>
</div>
...
<div class="formRow">
 <div class="formRight">
  ...
  <a href="index.php?com=page&act=man<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" onClick="if(!confirm('Bạn có muốn thoát không ? ')) return false;" title="" class="btn btn-secondary tipS ml-3" original-title="Thoát">Thoát</a>
</div>
<div class="clear"></div>
</div>
